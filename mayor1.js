...
Waiting:2. updateInventory([[2, "apples"], [5, "bananas"]], [[1, "apples"], [3, "bananas"], [4, "oranges"]]) should return [[3, "apples"], [8, "bananas"], [4, "oranges"]].
Waiting:3. updateInventory([], [[10, "apples"], [30, "bananas"], [20, "oranges"]]) should return [[10, "apples"], [30, "bananas"], [20, "oranges"]].
Waiting:4. updateInventory([[0, "Bowling Ball"], [0, "Dirty Socks"], [0, "Hair Pin"], [0, "Microphone"]], [[1, "Hair Pin"], [1, "Half-Eaten Apple"], [1, "Bowling Ball"], [1, "Toothpaste"]]) should return [[1, "Bowling Ball"], [0, "Dirty Socks"], [1, "Hair Pin"], [0, "Microphone"], [1, "Half-Eaten Apple"], [1, "Toothpaste"]].
 */

function updateInventory (inventory, shipment) {
    for(let i=0; i < inventory.length; i++){
        for (let j=0; j < shipment.length; j++) {
            if(inventory[i][1] !== shipment[j][1]){inventory.push(shipment[j])}
            else if (inventory[i][1] === shipment[j][1]){inventory.with([i][1],shipment[j][0]+inventory[i][0])}
            break
        };
    };
    return inventory;
